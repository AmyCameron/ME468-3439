...
slider_fric = chrono.ChMaterialSurfaceNSC()
slider_fric.SetFriction(0.4)

slider.GetCollisionModel().ClearModel()
slider.GetCollisionModel().AddBox(slider_fric, 0.2, 0.1, 0.1, chrono.VNULL, chrono.ChMatrix33D(chrono.QUNIT))
...
ball_fric = chrono.ChMaterialSurfaceNSC()

ball.SetCollide(True)
ball.GetCollisionModel().ClearModel()
ball.GetCollisionModel().AddSphere(ball_fric, 0.2, chrono.ChVectorD(0, 0, 0))
...
while (application.GetDevice().run()):
    ...
    if(0.01 < counter< 4.01):
      torque.append(engine_ground_crank.GetMotorTorque())
