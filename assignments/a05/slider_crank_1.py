...
motor_ground_crank = chrono.ChLinkMotorRotationSpeed()
motor_ground_crank.SetName("motor_ground_crank")
motor_ground_crank.Initialize(ground,crank,chrono.ChFrameD(chrono.ChVectorD(0, 0, 0), z2y))
motor_ground_crank.SetSpeedFunction(fun)
system.AddLink(motor_ground_crank)
...
spherical_rod_crank = chrono.ChLinkLockSpherical()
spherical_rod_crank.SetName("spherical_rod_crank")
spherical_rod_crank.Initialize(crank,rod, chrono.ChCoordsysD(chrono.ChVectorD(-2, 0, 0), chrono.QUNIT))
system.AddLink(spherical_rod_crank)
...
while (application.GetDevice().run()):
    ...
    print(motor_ground_crank.GetMotorTorque())

    if(counter<4.0):
      getpos_slider.append(slider.GetPos().x)

    if(0.01 < counter< 4.01):
      torque.append(motor_ground_crank.GetMotorTorque())
      counter_torque.append(counter_t)
      counter_t =  counter_t + 0.001
      
    if(4.0 < counter < 4.001):
      fig = plt.figure(figsize=(8, 6))
      plt.plot(counter_list,getpos_slider)
      plt.xscale("linear")
      plt.yscale("linear")
    ...
    if(4.001 < counter < 4.002):
      fig = plt.figure(figsize=(8, 6))
      plt.plot(counter_torque,torque)
      plt.xscale("linear")
      plt.yscale("linear")
      plt.xlabel("Time [Sec]",fontsize=12)
      plt.ylabel("Motor Torque [N-m]",fontsize=12)
      plt.show()
